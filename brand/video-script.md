...
- The wedge is **recompute-from-source**: it catches a wrong figure no matter how it got wrong, because it never compares to a stored hash. Demonstrate it with the tamper, then claim the wrong-when-written case **by construction** (the check is origin-blind). Do NOT claim a captured model self-error; there isn't one.
...
## 1:35 – 2:15 · The kill shot (the wedge)
**The catch (clean, repeatable, cannot fail on camera).**
**Screen:** a finished report, then the terminal (and/or the tamper caught live in the Band room).
**Say:** "Watch the auditor catch a wrong number. I doctor one figure in a finished report to a
value that looks perfectly honest." (Edit the `armada-claims` block: `gearing 0.26 → 0.01`.) Then run:
```
...
```
"No model in that verdict. It re-derived the number from source."

**Then the line that separates us from a hash (say it slowly):**
"Here is the point. This auditor recomputes from source. It never compares to a stored hash, so
it cannot tell whether that 0.01 was tampered in or written wrong by the model in the first
place. It catches both, the same way. A tamper-evident hash only catches the edit. It is blind to
a number that was wrong the moment it was written. We re-run the math, so we are not."

(The same tamper caught live on Band is in `band-session-export-tamper.json`; the CLI above is the bulletproof backup.)

## 2:15 – 2:35 · One control, any brain + posture
**Screen:** the `.env` `MODEL_PROVIDER` line.
**Screen:** the GLM and Claude clean summaries side by side (both 4 PASS, 2 refused).
...
- The two clean exports (GLM + Claude) ready for the portability beat; the tamper export or the
  CLI ready for the catch. There is no model-self-error artifact; do not claim one.
...
- **Lead correctness.** Show the recompute catching the doctored figure, then make the
  origin-blind point so it does not read as mere tamper-evidence. Do not claim a captured model self-error.
...
- `band-session-export-glm-clean.json` — GLM, 3-agent, all PASS (portability brain 1).
- `band-session-export-claude-clean.json` — Claude, 3-agent, all PASS (portability brain 2).
- `band-session-export-tamper.json` — clean run then tamper, AUDIT: FAIL (the catch).
